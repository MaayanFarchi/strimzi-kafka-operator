...
if [ -n "$KAFKA_CONNECT_SASL_MECHANISM" ]; then
    ...
    elif [ "$KAFKA_CONNECT_SASL_MECHANISM" = "custom" ]; then
        if [ -n "$KAFKA_CONNECT_SASL_LOGIN_CALLBACK_HANDLER_CLASS" ]; then
            OAUTH_CALLBACK_CLASS="sasl.login.callback.handler.class=$KAFKA_CONNECT_SASL_LOGIN_CALLBACK_HANDLER_CLASS"
        fi
        if [ -n "$KAFKA_CONNECT_SASL_JAAS_CONFIG" ]; then
            JAAS_CONFIG="$KAFKA_CONNECT_SASL_JAAS_CONFIG"
        fi
        if [ -n "$KAFKA_CONNECT_CUSTOM_SASL_MECHANISM" ]; then
            SASL_MECHANISM="$KAFKA_CONNECT_CUSTOM_SASL_MECHANISM"
        fi
        OAUTH_CALLBACK_CLASS_PRODUCER="producer.${OAUTH_CALLBACK_CLASS}"
        OAUTH_CALLBACK_CLASS_CONSUMER="consumer.${OAUTH_CALLBACK_CLASS}"
        OAUTH_CALLBACK_CLASS_ADMIN="admin.${OAUTH_CALLBACK_CLASS}"
